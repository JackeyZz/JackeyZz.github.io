...
public class Leaf{
  ...
  public static void main(String[] args){
    Leaf x = new Leaf();
    x.increment().increment().increment().print();  // output: i = 3
  }
}
```
...
```
上述程序执行过程：
要执行`main()`(静态方法)，必须加载`StaticInitialization`类，然后其静态域`table`和`cupboard`被初始化，这将导致对应的类也被加载，并且由于它们也都包含静态的`Bowl`对象，因为`Bowl`随后也被加载。这样，在这个特殊的程序中和所有类在`main()`开始之前都已被加载。
对象的创建过程，假设有个`Dog`类：
- 构造器其实也是静态方法，因此，当首次创建类型为`Dog`的对象时，或者`Dog`类的静态方法/静态域首次被访问时，java解释器必须查找类路径，以定位`Dog.class`文件。
- 然后载入`Dog.class`，有关静态初始化的所有动作都会执行。因此，静态初始化只在`Class`对象首次加载时进行一次。
- 当用`new Dog()`创建对象时，首先将在堆上为`Dog`对象分配足够的存储空间
- 这块存储空间将会被清零，这就自动地将`Dog`对象中的所有基本类型设置为初始化，引用设置为null
- 执行所有出现于字段定义处的初始化动作
- 执行构造器
...
### 显示的静态初始化
Java允许将多个静态初始化动作组织成一个特殊的“静态字句”，也称为“静态块”，即`static { }`。
### 非静态实例初始化
少了`static`关键字的代码块，用于支持“匿名内部类”的初始化，即`{ }`。
## 数组初始化
- 两个数组`a1 = a2`，表示a1和a2是相同数组的别名，指向同一个数组，对a2修改，在a1可以看到改变。
- `Integer[] a = {...}`与`Integer[] a = new Integer[]{...}`，前者只能用于数组被定义之处，后者可以在任何地方使用，包括方法调用内部
